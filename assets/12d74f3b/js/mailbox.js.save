	...
	$.yiimailbox.confirm = function(url)
	{
			...
			|| $.yiimailbox.confirmDelete==2)
		{
			if($.yiimailbox.currentFolder=='trash' || $.yiimailbox.trashbox==0)
			{
				...
				html = '<div id="dialog-confirm" title="Удалить навсегда?"><p><span class="ui-icon ui-icon-alert" style="float:left; margin:0 7px 20px 0;"></span>Эти сообщения будут удалены навегда. Вы уверены?</p></div>';
			}
			else {
				buttons = {
					"Удалить": function() {

...
class MessagingModule extends CWebModule
{
	...
	public static function t($str='',$params=array(),$dic='message') {
       ...
       public function getMessages($group) {
        ...
        public function getMessagesDp($groups) {
			$user_tb_name = call_user_func(array($this->userModel,'model'))->tableName();
            $criteria = new CDbCriteria;
            $criteria->condition="grp IN(".implode(',', $groups).")";
            $criteria->order="created DESC";
			$criteria->with = array('user');
			
			$messages = new CActiveDataProvider('Message', array(
				'criteria'=>$criteria,
				'pagination'=>array(
					'pageSize'=>100
				)
        ...
        public function getSentMessagesDp() {
            $criteria = new CDbCriteria;
            $criteria->condition="author_id=:usr";
			$criteria->params = array(":usr"=>$this->user->id);
            $criteria->order="created DESC";

...
class MailboxModule extends CWebModule
{
	 ...
	 */
	const TBL_CONV = 'tbl_mailbox_conversation';
	const TBL_MSG = 'tbl_mailbox_message';
	...
	*/
	 ...
	 */
	public $defaultSubject = '(без темы)';
	/**
	 ...
	 */
	public $allowableCharsSubject = '0-9a-zа-я.,!?@\s*$%#&;:+=_(){}\[\]\/\\-';
	/**
	...
	*/
	...
	{
		return Yii::app()->createUrl('users', array('public' => $userid));
	}
	...
	*/
	...
	public function getDate($time)
	{
		$timediff = time() - $time;
		if($timediff < 60 )
			$date = $timediff . ' сек назад';
		elseif($timediff < 3600 - 60) // within last hour
			$date = ceil($timediff / 60) . ' мин назад';
		...
		if (Yii::app()->user->isGuest) {
			if($controller->getId()!='news')
			{
				if (Yii::app()->user->loginUrl) {
					$controller->redirect($controller->createUrl('/users/login'));
				} else {

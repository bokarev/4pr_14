...
<?php echo CHtml::beginForm( $this->createUrl($this->getId().'/'.$this->getAction()->getId()), 'post', array('id'=>"message-list-form") ); ?>
...
<!-- <form id="message-list-form" action="<?php echo $this->createUrl($this->getId().'/'.$this->getAction()->getId()); ?>" method="post"> -->
	...
	if($dataProvider->getItemCount() > 1 && $this->getAction()->getId() != 'sent') : ?>
		<div class="btn-group mailbox-checkall-buttons">
			<button class="btn checkall">Выделить все</button>
			<button class="btn uncheckall">Снять выделение</button>
		</div>
	<?php
	endif;
...
$this->widget('zii.widgets.CListView', array(
	...
	array('created'=>'Дата отправления') :
	array('modified'=>'Дата прочтения'),
...
<div style="clear:left"> <span class="mailbox-buttons-label">With selected:</span>
	...
	<input type="submit" id="mailbox-action-restore" class="btn mailbox-button" name="button[restore]" value="восстановить" />
	<input type="submit" id="mailbox-action-delete" class="btn mailbox-button" name="button[delete]" value="удалить навсегда" />
		<?php else: ?>
			<?php if(!$this->module->readOnly || ( $this->module->readOnly && !$this->module->isAdmin()) ): ?>
	<input type="submit" id="mailbox-action-delete" class="btn mailbox-button" name="button[delete]" value="удалить" />
			<?php endif; ?>
	<input type="submit" id="mailbox-action-read" class="btn mailbox-button" name="button[read]" value="прочитан" />
	<input type="submit" id="mailbox-action-unread" class="btn mailbox-button" name="button[unread]" value="непрочит" />
		<?php endif; ?>
</div>
	<?php endif; ?>
	</div>

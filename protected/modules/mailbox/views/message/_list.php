...
switch($itemCssClass)
{
	case 'msg-read': $status = ($this->getAction()->getId()=='sent')? 'Получатель прочитал ваше сообщение' : 'Сообщение прочитано' ; break;
	case 'msg-deliver':  $status = 'Получатель не прочитал ваше сообщение';
	case 'msg-new': $status =  ($this->getAction()->getId()=='sent')? 'Получатель ещё не прочитал ваше сообщение' : 'Вы получили новое сообщение'; break;
	case 'msg-sent': $status = "Вы отправили {$username} сообщение";
}
...
<tr class="mailbox-item <?php echo $itemCssClass; ?> <?php if($this->getAction()->getId()!='sent') echo 'mailbox-draggable-row'; ?>">
    ...
    <td width="150" class="mailbox-received">
		<div align="right" class="mailbox-item-wrapper" style="width:80px">
			...
			<div class="mailbox-replied" title="На это сообщение ответили">&nbsp;&nbsp;</div>
			<?php endif; ?>
			<?php echo $received; ?>
		</div>

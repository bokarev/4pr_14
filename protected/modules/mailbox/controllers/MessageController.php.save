...
class MessageController extends Controller
{
	...
	public $buttons = array( // Ex output: {count} messages have been {value}
				'default'=>array('delete'=>'удалены','read'=>'отмечены как прочитанные','unread'=>'отмечены как непрочитанные'),
				'trash'=>array('delete'=>'удалены навсегда','restore'=>'восстановлены')
		);
	...
	public function actionTrash($ajax=null)
	{
		...
		Yii::app()->user->setFlash('notice', "Сообщения в корзине хранятся не более {$period} дней.");
	...
	public function actionNew()
	{
		...
		if(isset($_POST['Mailbox']['to']))
		{
			...
			if($conv->interlocutor_id && $conv->initiator_id == $conv->interlocutor_id) {
				$conv->addError('to', "Нельзя отправить сообщение самому себе!");
			}

			if(!$this->module->isAdmin() && $conv->interlocutor_id == $this->module->newsUserId){
				$conv->addError('to', "Пользователь не найден?");
			}
			...
			if(!$conv->hasErrors() && !$this->module->userToUser && !$this->module->isAdmin())
			{
				if(!$this->module->isAdmin($conv->to))
					$conv->addError('to', "Нет такого пользователя!");
			}
			...
			{
				Yii::app()->user->setFlash('error', "Ошибка при отправки сообщения!");
			}
		}
		else{
			$conv = new Mailbox();
